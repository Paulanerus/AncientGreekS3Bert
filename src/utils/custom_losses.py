...
from . import model_freeze as freeze

logger = logging.getLogger(__name__)


def cosine_sim(reps1: Tensor, reps2: Tensor):
    """cosine similarity"""
    return torch.nn.functional.cosine_similarity(reps1, reps2, dim=1)
...
class DistilLoss(nn.Module):
    ...
    def __init__(
        ...
        self.score_bias.to(model.device)
...
class MultipleConsistencyLoss(nn.Module):
    ...
    def forward(self, sentence_features: Iterable[Dict[str, Tensor]], labels: Tensor):
        ...
        teacher_reps = [
            self.teacher(sentence_feature)["sentence_embedding"]
            for sentence_feature in sentence_features
        ]
        ...
        teacher_scores = (
            self.similarity_fct(teacher_embeddings_a, teacher_embeddings_b) * self.scale
        )
